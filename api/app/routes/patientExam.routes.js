...
routePatientExam.get(
  "/api/patientexam/:VeterinarianVeterinarianId&:PatientPetPatientPetId",
...
// On post
routePatientExam.post(
  "/api/patientexam/get",
  [auth, viewer],
  async (request, response) => {
    var query =
      await `SELECT pe.*, pp.patientPetName, pp.patientPetGender, pp.patientPetBirthday ,
  ...
  async (request, response) => {
    ...
    mysqlConnection.getConnection(function (err, connection) {
      ...
      mysqlConnection.query(query, arrayValues, function (err) {
        ...
        fetch(
          process.env.EMAIL_API_ +
            "patientexam/" +
            jsonValues.VeterinarianVeterinarianId +
            "&" +
...
routePatientExam.delete(
  "/api/patientexam/:id",
  [auth, admin],
  async (request, response) => {
    var query = await `DELETE from ${process.env.MYSQL_D_B_}.PatientExams
               where patientExamId=?`;
    var values = await [parseInt(request.params.id)];
    mysqlConnection.getConnection(function (err, connection) {
      if (err) {
        response.status(501).json({
          message: apiMessage["501"][1],
