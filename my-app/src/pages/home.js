...
export class Home extends Component {
  constructor(props) {
    ...
    this.site3 = "state";
    this.site4 = "city";
    this.strongPassword = new RegExp(
      "(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[^A-Za-z0-9])(?=.{8,})"
    );
    this.mediumPassword = new RegExp(
  ...
  async refreshMedicalCenters() {
    console.log(
      `${REACT_APP_API_URL}${this.site2}/medicalcentername/${parseInt(
        this.state.medicalCenterId
      )}`
    );
        ...
        (data) => {
          ...
          if (this.state.medicalCenterNew !== 0) {
            this.getFromJson(
              ...
              "stateId",
              "stateName",
              this.state.medicalCenters.StateStateId
            ).then((value) => {
  ...
  async refreshCities(stateId) {
    console.log("refreshCities:", stateId);
    if (!stateId || parseInt(String(stateId)) === 0) stateId = await 5;
    await fetch(`${REACT_APP_API_URL}${this.site4}/${stateId}`, {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
      },
        ...
        (data) => {
          ...
          if (this.state.medicalCenterNew !== 0) {
            this.getFromJson(data,"cityId","cityName",this.state.medicalCenters.CityCityId
              ...
              console.log('MedicalCenterNew=',this.state.medicalCenterNew,' CityName:',this.state.cityName);
            });
          }
        },
        (error) => {
  ...
  onBlurPassword = async (e) => {
    ...
    await this.onBlurEmail(e);
    if ((await this.state.isLogin) === false) {
      await this.onBlurEmail(e);
      await this.onBlurMedicalCenter(e);
      await this.onBlurStateCity(e);
    }
    await this.fillValidateMessage();
  };
  ...
  onChangeMedicalCenterId = async (e) => {
    await this.setState({
      medicalCenterId: Math.abs(parseInt(e.target.value)),
    });
  ...
  onBlurMedicalCenterId = async (e) => {
    if ((await this.state.medicalCenterNew) !== 0) {
      await this.setState({
        ...
        medicalCenterTelNumber:
          this.state.medicalCenters.medicalCenterTelNumber,
  ...
  onBlurMedicalCenter = async (e) => {
    console.log(
      `onBlurMedicalCenter:${this.state.medicalCenterId}, ${this.state.medicalCenterName}, 
      ${this.state.medicalCenterAddress}, ${this.state.medicalCenterTelNumber}`
    );
    if(!this.state.medicalCenterId || !this.state.medicalCenterName || !this.state.medicalCenterAddress || !this.state.medicalCenterTelNumber){
      this.state.arrayValidate[2] = false;
    }else {(String(this.state.medicalCenterId).length > this.lim &&
      this.state.medicalCenterName.length > this.lim &&
      this.state.medicalCenterAddress.length > this.lim &&
      String(this.state.medicalCenterTelNumber).length > this.lim) ||
    this.state.medicalCenterNew !== 0
      ? (this.state.arrayValidate[2] = true)
      : (this.state.arrayValidate[2] = false);}
    
    await this.onBlurEmail(e);
    await this.fillValidateMessage(e);
  };
  ...
  onChangeMedicalCenterTelNumber = async (e) => {
    await this.setState({
      medicalCenterTelNumber: Math.abs(parseInt(e.target.value)),
    });
  };
  ...
  onBlurStateCity = async (e) => {
    ...
    console.log('onBlurStateCity:', e.target.name);
    if(e.target.name==='StateStateId') await this.refreshCities(this.state.StateStateId);
    await this.onBlurEmail(e);
    await this.onBlurMedicalCenter(e);
    await this.fillValidateMessage(e);
  };
  ...
  onChangeAdmin = async (e) => {
    ...
    console.log(
      "medId",
      this.state.medicalCenterId.toString().length,
      "medNa",
      this.state.medicalCenterName.length
    );
    console.log(
      "medAd",
      this.state.medicalCenterAddress.length,
      "medTe",
      this.state.medicalCenterTelNumber.toString().length
    );
    console.log(
      "compa",
      this.state.password === this.state.passwordAgain,
      "Stre",
      this.state.strengthBadge
    );
    console.log(
      "Check",
      this.state.Viewer ||
        this.state.Clinic ||
        this.state.Laboratory ||
        this.state.Admin
    );
    console.log(
      "Depar",
      this.state.StateStateId,
      "Ciud",
      this.state.CityCityId
    );
  };
  ...
  onBlurUserType = async (e) => {
    ...
    await this.onBlurStateCity(e);
    await this.onBlurPassword(e);
    await this.onBlurPasswordAgain(e);
    await this.fillValidateMessage();
  };
  ...
  validateForm() {
    try {
      return this.state.isLogin
        ? this.state.email.length > this.lim &&
            this.state.password.length > this.lim
        : this.state.email.length > this.lim &&
            this.state.password.length > this.lim &&
            String(this.state.medicalCenterId).length > this.lim &&
            this.state.medicalCenterName.length > this.lim &&
            this.state.medicalCenterAddress.length > this.lim &&
            String(this.state.medicalCenterTelNumber).length > this.lim &&
            this.state.password === this.state.passwordAgain &&
            ...
            this.state.StateStateId > 1 &&
            this.state.CityCityId > 1000;
    } catch (e) {
      console.error(e);
      return false;
    }
                    ...
                    if (!this.state.auth.token) {
                      ...
                      !this.state.auth.message
                        ? !this.state.auth.error
                          ...
                          : alert(this.state.auth.error)
                        : alert(this.state.auth.message);
                    } else {
                      localStorage.setItem(
                        "Token",
                        JSON.stringify(this.state.auth)
                      );
                      this.setState({
                        Token: JSON.parse(localStorage.getItem("Token")),
                      });
              ...
              if (!this.isFound) {
                ...
                fetch(`${REACT_APP_API_URL}${this.site}/signup`, {
                  ...
                  body: JSON.stringify({
                    ...
                    medicalCenterTelNumber: parseInt(
                      this.state.medicalCenterTelNumber
                    ),
              ...
              <Modal.Body className="Login">
                <Form onSubmit={this.handleSubmit}>
                  <div>
                    ...
                    {!isLogin ? (
                      <Form.Group size="lg" className="form-group required">
                        ...
                        <Form.Control
                          ...
                          onChange={this.onChangeMedicalCenterId}
                          placeholder="Nit Centro Médico"
                          onBlur={this.onBlurMedicalCenterId}
                          id="medicalCenterId"
                          required="required"
                        />
                        {medicalCenterNew === 0 ? (
                          <div>
                            <Form.Control
                              ...
                              onChange={this.onChangeMedicalCenterName}
                            ...
                            <Form.Control
                              ...
                              onChange={this.onChangeMedicalCenterAddress}
                              onBlur={this.onBlurMedicalCenter}
                            ...
                            <Form.Control
                              ...
                              value={medicalCenterTelNumber}
                              id="medicalCenterTelNumber"
                              onChange={this.onChangeMedicalCenterTelNumber}
                              onBlur={this.onBlurMedicalCenter}
                              name="medicalCenterTelNumber"
                            />
                            <div className="input-group mb-3">
                              <select
                                ...
                                name="StateStateId"
                              >
                                <option hidden defaultValue value="0" key="0">
                                  Departamento
                                </option>
                                {states.map((sta) => (
                                  <option value={sta.stateId} key={sta.stateId}>
                                    {sta.stateName}
                                  </option>
                                ))}
                              </select>
                              ...
                              >
                                ...
                                {cities.map((cit) => (
                                  <option value={cit.cityId} key={cit.cityId}>
                                    {cit.cityName}
                                  </option>
                                ))}
                              </select>
                          ...
                          <div>
                            <Form.Control
                              type="name"
                              className="form-control"
                              value={medicalCenters.medicalCenterName}
                              readOnly
                              onBlur={this.onBlurMedicalCenter}
                            />
                            <Form.Control
                              type="name"
                              className="form-control"
                              value={medicalCenters.medicalCenterAddress}
                              readOnly
                              onBlur={this.onBlurMedicalCenter}
                            />
                            <Form.Control
                              type="name"
                              className="form-control"
                              value={medicalCenters.medicalCenterTelNumber}
                              readOnly
                              onBlur={this.onBlurMedicalCenter}
                            />
                            <div className="input-group mb-3">
                    ...
                    {!isLogin ? (
                      <Form.Group size="lg" className="form-group required">
                        <span id="StrengthDisp" className={backgroundColor}>
                          {strengthBadge}
                        </span>
                        <Form.Label className="control-label">
                        ...
                        <Form.Control
                          ...
                          onBlur={this.onBlurPasswordAgain}
                          autoComplete="current-password"
                          id="passwordAgain"
                          required="required"
                        />
                          ...
                          <div key={`inline-${type}`} className="mr-3">
                            ...
                            {disabledArray[2] ? (
                              ...
                              <Form.Check
                                ...
                                type={type}
                                id={`inline-${type}-3`}
                                onChange={this.onChangeLaboratory}
                                checked={Laboratory}
                                onBlur={this.onBlurUserType}
                              />
                            )}
