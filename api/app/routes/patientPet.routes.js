...
// On get
routePatientPet.get(
  "/api/patientpet/:medicalCenterArray",
  [auth, viewer],
...
// On post
routePatientPet.post(
  "/api/patientpet/getpatientpetid",
  [auth, viewer],
  async (request, response) => {
...
routePatientPet.post(
  ...
  async (request, response) => {
    ...
    if (
      jsonValues.patientPetBirthday.length < 3 ||
      jsonValues.patientPetBirthday[0] === ""
    ) {
      jsonValues.patientPetBirthday = null;
    } else {
      jsonValues.patientPetBirthday = new Date(
