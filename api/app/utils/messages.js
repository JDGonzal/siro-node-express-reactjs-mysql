const apiMessage =
{
  '201': ['Added Successfully.',
    'Adicionado correctamente.'],
  '202': ['Updated Successfully.',
    'Actualización Exitosa.'],
  '203': ['This account was activated sucessfully.',
    'Esta cuenta fue activada correctamente.'],
  '204': ['Please check your email for activate this account.',
    'Por favor verificar su correo para activar esta cuenta.'],
  '400': ['Validation Failed.',
    'La validación Falló.'],
  '401': ['Invalid email or password.',
    'El correo electrónico o la contraseña están errados.'],
  '403': ['Invalid password or email.',
    'Dato incorrecto de Contraseña o Correo Electrónico.'],
  '404': ['Empty data.',
    'Datos vacíos.'],
  '409': ['Error: The user could not exist or was activated previoulsy.',
    'Error: El usuario no existe o fue activado previamente.'],
  '501': ['Something went wrong.',
    'Algo ha ido mal...'],
  '511': ['Account is pending for activate.',
    'La cuenta está pendiente por ser activada.'],
};

module.exports = apiMessage;